...
Y = X;
SIG_W = 0.01; % STATE NOISE VARIANCE
...
for i = 2:L % SERIAL ARRIVAL OF OBSERVATION VECTORS
    ...
    h = figure; 
    B = X; plot(B(1,i),B(2,i),'o','MarkerSize',7,'MarkerEdgeColor','red', 'MarkerFaceColor', 'red' ); hold on;
    B = X_hatA;plot(B(1,i),B(2,i),'d','MarkerSize',7,'MarkerEdgeColor','red', 'MarkerFaceColor', 'green' ); hold on;
    B = X_hatB; plot(B(1,i),B(2,i),'d','MarkerSize',7,'MarkerEdgeColor','red', 'MarkerFaceColor', 'blue' ); hold on;
    xlabel('x(m)'); ylabel('y(m)'); title(strcat('Tracking of positional variable')); % --Iteration no ',' ', num2str(i-1)))
    legend('True Position','Tracker 1','Tracker 2');
    legend('True Position','Tracker 1','Tracker 2');
    axis([-ax ax -ax ax])
    
    saveas(h,sprintf('FIG1v%d.png',i-1));
    hold off;
end
